...
interface RaindropClient {
	getRandomArticle(collectionId: string): Promise<RaindropItem>;
}

/**
 * Creates a Raindrop.io API client with the provided token
 * @param token - The Raindrop.io API token
 * @returns A client instance for interacting with Raindrop.io API
 */
export function createRaindropClient(token: string): RaindropClient {
	...
	return {
		...
		async getRandomArticle(collectionId: string): Promise<RaindropItem> {
			const url = `${BASE_URL}/raindrops/${collectionId}`;
